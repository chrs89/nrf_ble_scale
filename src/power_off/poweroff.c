...
#define SYSTEM_ON_LED DK_LED4

#define UART_WAIT_MS 50
#define UART_WAIT_RETRIES 10

LOG_MODULE_REGISTER(POWEROFF, LOG_LEVEL_DBG);
    ...
    if (IS_ENABLED(CONFIG_PM_DEVICE) && IS_ENABLED(CONFIG_SERIAL))
    {
        ...
        if (dev)
        {
            do
            {
                ...
                k_sleep(K_MSEC(UART_WAIT_MS));
                retries++;
            } while (retries < UART_WAIT_RETRIES);

            if (state == PM_DEVICE_STATE_ACTIVE)
            {
                LOG_WRN("UART still active after timeout. Forcing system off.");
            }
        }
        else
        {
            LOG_WRN("Console device not found.");
        }
    }
